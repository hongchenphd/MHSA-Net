...
class TripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False):
        ...
        #print(dist_mat)
        #dist_mat1=self.max_pool(dist_mat.unsqueeze(0)).squeeze()
        #print(dist_mat1)
...
def triplet_ranking_loss( A, B, I, max_dim):
    loss = torch.sum((1.0 + A - B))
...
class FDRT(nn.Module):     #####   Diversity
    """Cross entropy loss with label smoothing regularizer.
    Reference:
    Szegedy et al. Rethinking the Inception Architecture for Computer Vision. CVPR 2016.
    Equation: y = (1 - epsilon) * y + epsilon / K.
    ...
    def forward(self, inputs, targets):
        I=torch.eye(inputs.shape[2]).cuda()#-0.1*torch.eye(inputs.shape[2]).cuda()+0.1
        #I=torch.zeros(23).cuda() 
        inter=torch.sum(pow((inputs-I),2.0),1)
        inter=torch.sum(inter,1)
        inter=(1.0/25.0)*pow(inter, 0.5)
        att0=torch.sum(inter)
        return 0.0001*att0  


class IHTL(object):  #  93.8%   82.9%
    """Modified from Tong Xiao's open-reid (https://github.com/Cysu/open-reid).
    Related Triplet Loss theory can be found in paper 'In Defense of the Triplet
    Loss for Person Re-Identification'."""
    ...
    def __call__(self, img, labels, normalize_feature=False):   #def forward(self, img, txt, global_feat, labels):
        
        if normalize_feature:
           img = normalize(img, axis=-1)
        img=img.view(-1, img.size(-1))
        scores= euclidean_dist(img, img)
        dist_mat=self.max_pool(scores.unsqueeze(0)).squeeze() 
        dist_ap, _  = hard_example_mining(dist_mat, labels, self.margin)
        dist_mat=self.max_pool(-1.0*scores.unsqueeze(0)).squeeze()
        _, dist_an = hard_example_mining(-1.0*dist_mat, labels, self.margin)
        y = dist_an.new().resize_as_(dist_an).fill_(1)
        loss = self.ranking_loss(dist_an - dist_ap, y)
        return   1.0*loss 
...
class ACM(nn.Module):    
    ...
    def forward(self, inputs, targets):
        inputs= torch.sum(inputs[inputs<0.01])
        return 0.0001*inputs
