...
from models.networks import ResNetBuilder, IDE, Resnet, MHSA
from trainers.evaluator import ResNetEvaluator
from trainers.trainer import cls_tripletTrainer
from utils.loss import CrossEntropyLabelSmooth, TripletLoss, Margin, FDRT, IHTL, ACM
...
def train(**kwargs):
   ...
   # os.makedirs(opt.save_dir, exist_ok=True)
    ...
    elif opt.model_name == 'MHSA':
        if opt.datatype == "person":
            model = MHSA(dataset.num_train_pids, 1.0, 0.33)
        else:
            model = MHSA(dataset.num_train_pids, 0.5, 0.5)
    ...
    xent_criterion1 = FDRT(dataset.num_train_pids)
    xent_criterion2 = IHTL(dataset.num_train_pids)
    xent_criterion3 = ACM(dataset.num_train_pids)
    ...
    def criterion(softmax_y2, softmax_y1, hongchen, triplet_y, softmax_y, labels):
        losses = [embedding_criterion(output, labels)[0] for output in triplet_y] + \
                     [xent_criterion(output, labels) for output in softmax_y]+  [xent_criterion1(output, labels) for output in softmax_y1] +[xent_criterion2(output, labels) for output in softmax_y2]+[xent_criterion3(output, labels) for output in hongchen]
